  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 md:p-6 font-display overflow-hidden">
      ...
      <div className="relative z-50 flex w-full max-w-[95%] sm:max-w-lg flex-col overflow-hidden rounded-2xl bg-white border border-primary/40 shadow-2xl dark:bg-background-dark animate-fade-in-up transition-all duration-300 max-h-[92vh]">
        ...
        <main className="flex-1 overflow-y-auto sidebar-scroll p-5 sm:p-8">
          <div className="flex flex-col items-center space-y-6 sm:space-y-8">
            
            {/* Header Section */}
            <div className="text-center w-full px-2">
              <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-text-main dark:text-white uppercase tracking-[0.05em]">Merchant Profile</h2>
              <p className="text-xs sm:text-sm text-text-secondary dark:text-gray-400 mt-1">Manage your professional store identity</p>
            </div>

            {/* Avatar Section: Responsive sizing */}
            <div className="relative group">
              <div className="size-24 sm:size-32 rounded-full border-4 border-white shadow-xl overflow-hidden dark:border-[#2a3837]">
                <div 
            ...
            <form className="w-full space-y-4 sm:space-y-5" onSubmit={(e) => e.preventDefault()}>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative flex items-center group">
                  ...
                  <input 
                    className="w-full rounded-xl border border-border-color bg-slate-50 py-3 sm:py-3.5 pl-10 pr-12 text-sm sm:text-base text-text-main placeholder-text-secondary focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary dark:border-[#2a3837] dark:bg-[#1a2625] dark:text-white transition-all font-medium" 
                    type="email" 
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative group">
                  <span className="material-symbols-outlined absolute left-3 top-4 text-text-secondary group-focus-within:text-primary transition-colors text-xl">description</span>
                  <textarea 
                    className="w-full rounded-xl border border-border-color bg-slate-50 py-3 sm:py-3.5 pl-10 pr-4 text-sm sm:text-base text-text-main placeholder-text-secondary focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary dark:border-[#2a3837] dark:bg-[#1a2625] dark:text-white transition-all font-medium min-h-[80px] sm:min-h-[100px] resize-none" 
            ...
            <div className="w-full grid grid-cols-2 gap-3 sm:gap-4">
              <button 
                ...
                className="flex flex-col items-center justify-center rounded-2xl border border-border-color bg-white p-4 sm:p-5 text-center shadow-sm transition-all hover:border-primary/50 dark:border-[#2a3837] dark:bg-[#1a2625] hover:shadow-md active:scale-[0.98]"
              >
                <div className="mb-2 flex size-8 sm:size-10 items-center justify-center rounded-full bg-primary/10 text-primary dark:bg-primary/20">
                  <span className="material-symbols-outlined text-[20px] sm:text-[22px]">photo_library</span>
                </div>
                <h4 className="text-xl sm:text-2xl font-black text-text-main dark:text-white leading-none">2,845</h4>
                <p className="text-[8px] sm:text-[10px] font-black text-text-secondary dark:text-gray-400 uppercase tracking-[0.1em] mt-1">Creations</p>
              </button>
              <button 
              ...
              >
                ...
                <h4 className="text-xl sm:text-2xl font-black text-text-main dark:text-white leading-none">124</h4>
                <p className="text-[8px] sm:text-[10px] font-black text-text-secondary dark:text-gray-400 uppercase tracking-[0.1em] mt-1">Active Days</p>
              </button>
            </div>

            {/* Pro Status Banner: Responsive layout */}
            <div className="w-full relative overflow-hidden rounded-2xl bg-gradient-to-r from-primary/20 to-primary/5 p-4 sm:p-6 shadow-sm dark:from-primary/10 dark:to-transparent border border-primary/20">
            ...
            <div className="w-full pt-2">
              <button 
                onClick={handleSave}
                className="w-full rounded-xl bg-primary py-3.5 sm:py-4 text-sm font-black text-slate-900 shadow-xl shadow-primary/20 transition-all hover:bg-secondary hover:-translate-y-0.5 active:translate-y-0 active:scale-95"
              >
                Save Changes
              </button>
            </div>
