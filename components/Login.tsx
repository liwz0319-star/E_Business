...
const Login: React.FC<LoginProps> = ({ onLogin, onSignup }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // MEDIUM-003: 使用统一错误处理
      setErrorMessage(handleAuthError(error, 'login'));
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="relative z-10 flex h-full w-full flex-col justify-end p-12">
          <div className="max-w-lg">
            ...
            <p className="text-lg text-white/80 font-medium">
              Create studio-quality product photography and videos in seconds with our AI engine.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="mx-auto w-full max-w-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="flex w-full justify-center rounded-lg bg-primary px-3 py-3.5 text-sm font-bold leading-6 text-slate-900 shadow-sm hover:bg-primary-hover transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <>
